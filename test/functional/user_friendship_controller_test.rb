...
class UserFriendshipControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendhsip, user: users(:Mam), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:Mam), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
        @friendship3 = create(:requested_user_friendship, user: users(:Mam), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
        @friendship4 = user_friendships(:blocked_by_Mam)
      ...
      context "blocked users" do
        ...
        should "get the index wihout error" do
          assert_response :success
        end
  ...
  context "#new" do
    ...
    context "when logged in" do
    	...
    	should "should set a flash error if the fiend_id params is missing" do
    		get :new, {}
    	...
    	should "assign a new user friendship to the currently logged in user" do
    		get :new, friend_id: users(:jim)
    		assert_equal users(:Mam), assigns(:user_friendship).user
    	end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:Mam), friend: @friend)
        create(:pending_user_friendship, friend: users(:Mam), user: @friend)
        sign_in users(:Mam)
      ...
      should "assign a user_friendship" do
        assert assigns(:user_friendship)
        assert_equal @user_friendship, assings(:user_friendship)
      end
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:Mam))
        sign_in users(:Mam)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:Mam))
        create(:accepted_user_friendship, friend: users(:Mam), user: @friend)
        sign_in users(:Mam)
        
      end

      should "delete user frienships" do
        assert_difference 'UserFriendship.count', 2 do
